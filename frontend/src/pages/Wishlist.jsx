...
const Wishlist = () => {
  ...
  useEffect(() => {
    ...
    const fetchWishlistCountries = async () => {
      setIsLoading(true);
      try {
        // Fetch user's wishlist country names
        ...
        // Filter only the wishlist countries
        const wishlistCountriesDetails = countriesResponse.data.filter(country => 
          wishlistResponse.data.includes(country.name.common)
        );
        
        setCountries(wishlistCountriesDetails);
      } catch (error) {
        console.error('Error fetching wishlist countries:', error);
        if (error.response && error.response.status === 401) {
          navigate('/login', { state: { message: 'Session expired. Please log in again.' } });
        } else {
          setError('Failed to fetch wishlist countries. Please try again later.');
        }
      } finally {
        setIsLoading(false);
      }
  ...
  const handleRemoveFromWishlist = async (e, countryName) => {
    ...
    try {
      ...
      // Update local state to remove the country from wishlist
      ...
      <main className="flex-grow p-4 md:p-8">
        <div className="max-w-7xl mx-auto">
          <div className="mb-10 text-center">
            <h1 className="text-4xl md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-600 mb-2">
              My Travel Wishlist
            </h1>
            <p className="text-gray-600 max-w-2xl mx-auto">
              Countries you plan to visit in the future. Start planning your next adventure!
            </p>
          </div>
            ...
            <>
              {countries.length > 0 ? (
                    ...
                    >
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute top-2 right-2">
                          <button
                            onClick={(e) => handleRemoveFromWishlist(e, country.name.common)}
                            className="p-2 rounded-full bg-indigo-500 text-white shadow-md transition-all duration-200 hover:bg-indigo-600"
                            aria-label={`Remove ${country.name.common} from wishlist`}
                          >
                ...
                <div className="text-center py-16">
                  ...
                  <p className="mt-1 text-gray-500">Explore countries and add them to your travel wishlist by clicking the bookmark icon.</p>
                  <button 
                    onClick={() => navigate('/')}
